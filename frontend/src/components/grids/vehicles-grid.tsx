...
export default function VehiclesGrid() {
	...
	const filteredVehicles = vehicles.filter((vehicle) => [vehicle.brand, vehicle.model, vehicle.plateNumber, vehicle.fuelType, vehicle.status].some((field) => field?.toLowerCase().includes(search.toLowerCase())));
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{loading ? (
					...
					filteredVehicles.map((vehicle, idx) => (
						<Card key={vehicle.id} className={idx % 2 === 0 ? "bg-muted/40 hover:bg-muted" : "bg-muted/0 hover:bg-muted"}>
							<CardHeader>
