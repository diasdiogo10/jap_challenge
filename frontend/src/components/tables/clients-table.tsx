...
export default function ClientsTable() {
	...
	const filteredClients = clients.filter((client) => [client.fullName, client.email, client.phone, client.drivingLicense].some((field) => field?.toLowerCase().includes(search.toLowerCase())));
				...
				<TableBody>
					{loading
						...
						: filteredClients.map((client, idx) => (
								<TableRow key={client.id} onClick={() => router.push(`/clients/${client.id}`)} className={idx % 2 === 0 ? "bg-muted/40 hover:bg-muted" : "bg-muted/0 hover:bg-muted"}>
